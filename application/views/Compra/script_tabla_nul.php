...
var tabla;
$( "#ANUL,#Compra" ).addClass( "active" );
$( "#ANUL,#A_N_L" ).addClass( "text-red" );
$( "#accion" ).addClass( "fa fa-plus-square" );
  function format ( d ) {
      var id     =  d.slice(4) ;
      $.ajax({
      type : 'POST',
      url: "<?php echo base_url('index.php/Compra/detale');?>/"+id,
      dataType: 'json',
      })
      .done(function(data) {
        if (data) {
...
$(function() {
    tabla = $('#tabla_nul').DataTable({
        ...
        "ajax": {
            "url": "<?php echo base_url('index.php/Compra/anulados'); ?>",
            "type": "POST"
        },
