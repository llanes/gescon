let motopagar = 0, final = 0, pagoEfectivo = 0, pagoCeque = 0, pagoTarjeta = 0, saldoFavor = 0;
let bandera = true, banderas = true;
let proveedor, table, save_method;
const $imagenContenedor = $('.img-thumbnail'), $tablaProductos = $('#tablaProductos');
const ruta_de_la_imagen = "<?=base_url('bower_components/uploads/')?>", srcImagenNoDisponible = ruta_de_la_imagen + 'imgnull.jpg';
let typingTimer, timeout = null;
...
$('#campoBusqueda').on('input', function() {
  ...
  $("#proveedor").select2({
    ...
    if ($(this).val()) {
      var id = $(this).val();
      if (id == '') {id = 0;}
         $( ".orden" ).load( "Compra/list_orden/"+id);
         if (!$.isEmptyObject(id)) {
...
/////////////////////////////seccion del carrito//////////////////////////////
    ...
    function update2_rowid(price,id,qty,iva,tyf) {
      if (price !== '' && price !== 0 ) {
        var parametro = {}
        $.ajax({
          url : "Orden_compra/update2_rowid",
          type: "POST",
          dataType: "JSON",
          data: {
            price : price,
